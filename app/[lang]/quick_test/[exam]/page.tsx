...
const EXAM_DURATION_SECONDS = 120; // 2 minutes

export default function ExamPage({ params }: { params: Promise<{ lang: string; exam: string }> }) {
    const { lang, exam } = React.use(params);
    const router = useRouter();
    const t = dictionary[lang as Lang];
    const examId = parseInt(exam, 10);
    ...
    // Helper: Generate a chunk of random words from the pool
    ...
    // Load Exam Data
    const poolDataObj = (quickTestData as Record<string, any>)[`exam_${examId}_pool`];

    useEffect(() => {
        if (poolDataObj && poolDataObj.pool) {
            setLessonTitle((lang === 'en' && poolDataObj.title2) ? poolDataObj.title2 : poolDataObj.title);
            // Initial generation of drills
    ...
    // Generate more lines dynamically if user is fast
    useEffect(() => {
        if (currentDrillIndex >= drills.length - 5 && poolDataObj) {
            setDrills(prev => [...prev, ...generateMoreDrills(poolDataObj.pool, 10)]);
        }
        ...
        <div className="h-screen max-h-screen bg-slate-50 flex flex-col items-center justify-start py-2 px-4 overflow-hidden select-none font-sarabun" onClick={() => inputRef.current?.focus()}>
            ...
            <div className="w-full max-w-4xl flex items-center justify-between mb-4 mt-4">
                <Link href={`/${lang}/quick_test`} className="text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-2 font-medium">
                    ...
                    {(dictionary[lang as Lang] as any).lessons?.quickTestSection?.exitTest || "Exit Test"}
                </Link>

                <div className="flex items-center gap-4 bg-white px-6 py-2 rounded-xl shadow-sm border border-slate-200">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-sm">{((dictionary[lang as Lang] as any).lessons?.scoreboard?.time_label) || "Timer"}</span>
                    <span className={`text-2xl font-black font-mono w-[80px] text-right ${timeLeft <= 10 ? 'text-red-500 animate-pulse' : 'text-indigo-600'}`}>
                        {timerDisplayStr}
                    </span>
                </div>

                <div className="w-[100px]" /> {/* Spacer for centering timer */}
            </div>
