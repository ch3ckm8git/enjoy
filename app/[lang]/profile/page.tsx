...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            ...
            if (docSnap.exists()) {
                ...
                setUserStats(data);
                setUsername(data.username || "New Learner");
                setTempUsername(data.username || "New Learner");
            }
            setIsLoading(false);
        };
        fetchUserData();
    ...
    const handleSave = async () => {
        if (!tempUsername.trim() || !user) return;
                                        ...
                                        <div className="px-8 pb-8">
                                            ...
                                            <div className="flex justify-between items-end -mt-16 mb-6">
                                                <div className="relative group/avatar">
                                                    <div className="w-32 h-32 rounded-3xl bg-white p-1.5 shadow-xl ring-1 ring-slate-900/5 relative z-10">
                                                ...
                                                {!isEditing && (
                                                    <div className="flex gap-3">
                                                        <button
                                                            onClick={logout}
                                                            className="px-5 py-2.5 bg-red-50 border border-red-200 text-red-600 font-semibold rounded-xl shadow-sm hover:bg-red-100 hover:border-red-300 transition-all flex items-center gap-2"
                                                        >
                                                            {t.logout || "Log out"}
                                                        </button>
                                                        <button
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex items-center gap-3 px-4 py-3 bg-slate-50/50 rounded-xl border border-transparent">
                                                        ...
                                                        <span className="font-medium text-slate-600 text-lg">{userStats.lastActiveDate ? formatDate(userStats.lastActiveDate.toDate ? userStats.lastActiveDate.toDate().toISOString() : userStats.lastActiveDate) : "Today"}</span>
                                                    </div>
                                                </div>
